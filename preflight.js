(()=>{
  const TRADE_KEY='tradepilot-trades-v2', PLAN_KEY='tradepilot-plan-v1';
  ...
  try{
    const raw=localStorage.getItem(TRADE_KEY);const data=raw?JSON.parse(raw):[];
    localStorage.setItem(TRADE_KEY,JSON.stringify(Array.isArray(data)?data.map(cleanTrade):[]));
  }catch{localStorage.setItem(TRADE_KEY,'[]')}
  try{
    const raw=localStorage.getItem(PLAN_KEY);const p=raw?JSON.parse(raw):{};
    const safe={symbol:cleanText(p?.symbol).slice(0,40).toUpperCase(),entry:cleanText(p?.entry).slice(0,50),sl:cleanText(p?.sl).slice(0,50),tp:cleanText(p?.tp).slice(0,50),bias:['LONG','SHORT','ESPERAR'].includes(p?.bias)?p.bias:'LONG',thesis:cleanText(p?.thesis)};
    localStorage.setItem(PLAN_KEY,JSON.stringify(safe));
  }catch{localStorage.setItem(PLAN_KEY,'{}')}
})();
