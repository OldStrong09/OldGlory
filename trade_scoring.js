(()=>{
  ...
  function score(t,all){
    let points=0,errors=[];
    const sl=n(t.sl),tp=n(t.tp),entry=n(t.entry),rr=n(t.rr),result=n(t.result),risk=riskPerTrade();
    ...
    const same=all.filter(x=>String(x.strategy||'').trim()===String(t.strategy||'').trim());const recent=same.slice(-3);if(recent.length===3&&recent.every(x=>n(x.result)!==null&&n(x.result)<0))errors.push('3 pérdidas seguidas en estrategia');
    return {score:Math.max(0,Math.min(100,points)),errors};
  }
  function decorate(){
    const table=$('#journal-table');if(!table)return;const rows=[...table.querySelectorAll('tbody tr')];if(!rows.length)return;
    const data=read().slice().reverse();const head=table.querySelector('thead tr');
    ...
    rows.forEach((row,i)=>{if(row.querySelector('.trade-score'))return;const t=data[i];if(!t)return;const x=score(t,data);const td=document.createElement('td');td.className='trade-score';td.innerHTML=`<strong>${x.score}/100</strong>${x.errors.length?`<div class="muted">${esc(x.errors.slice(0,2).join(' · '))}${x.errors.length>2?'…':''}</div>`:'<div class="muted">Sin errores clave</div>'}`;row.insertBefore(td,row.lastElementChild)});
  }
  function patterns(){
    const a=read(),closed=a.filter(t=>n(t.result)!==null),errors={};closed.forEach(t=>score(t,closed).errors.forEach(e=>errors[e]=(errors[e]||0)+1));
    return Object.entries(errors).sort((a,b)=>b[1]-a[1]).slice(0,5);
  }
  function renderPanel(){
    const host=$('#view-journal');if(!host||$('#trade-quality-panel'))return;const c=document.createElement('div');c.id='trade-quality-panel';c.className='card';c.innerHTML='<div class="section-head"><div><h2>Control de calidad del proceso</h2><p>La puntuación mide la calidad de ejecución y registro; no premia ganar dinero.</p></div></div><div class="hero-grid" id="quality-grid"></div><div class="two-col"><div><h3>Errores repetidos</h3><div id="quality-errors" class="insight"></div></div><div><h3>Regla de mejora</h3><div class="insight">Un trade ganador con mala disciplina sigue siendo un trade de mala calidad. Corrige el proceso antes de aumentar el riesgo.</div></div></div>';host.appendChild(c);updatePanel()}
  function updatePanel(){const a=read(),closed=a.filter(t=>n(t.result)!==null),scores=closed.map(t=>score(t,closed).score),avg=scores.length?scores.reduce((x,y)=>x+y,0)/scores.length:0,good=scores.filter(x=>x>=80).length;const grid=$('#quality-grid'),err=$('#quality-errors');if(grid)grid.innerHTML=`<div class="card"><div class="card-label">CALIDAD MEDIA</div><div class="metric">${scores.length?Math.round(avg)+'/100':'—'}</div><div class="muted">disciplina registrada</div></div><div class="card"><div class="card-label">TRADES ≥ 80</div><div class="metric">${scores.length?good+'/'+scores.length:'—'}</div><div class="muted">alta calidad de proceso</div></div><div class="card"><div class="card-label">ERRORES RECURRENTES</div><div class="metric">${patterns().reduce((s,x)=>s+x[1],0)||0}</div><div class="muted">incidencias detectadas</div></div>`;if(err){const p=patterns();err.innerHTML=p.length?p.map(x=>`<div>• ${esc(x[0])}: <strong>${x[1]}</strong></div>`).join(''):'<div>✓ No hay errores recurrentes detectables.</div>'}}
  let last='';function refresh(){const snap=JSON.stringify(read());if(snap===last)return;last=snap;decorate();renderPanel();updatePanel();window.dispatchEvent(new Event('apex-data-updated'))}
  function init(){refresh();const obs=new MutationObserver(()=>refresh());const j=$('#journal-table');if(j)obs.observe(j,{childList:true,subtree:true});setInterval(refresh,800)}
  window.tradePilotScoring={score,patterns,refresh};
