(()=>{
 ...
 function status(){const s=read(),ks=window.tradePilotKillSwitch?.isArmed?.()||false;return {...s,killSwitch:ks||s.killSwitch,liveLocked:true,liveExecutionEnabled:false}};
 ...
 function load(src){if(document.querySelector('script[src="'+src+'"]'))return;const s=document.createElement('script');s.src=src;s.defer=false;document.body.appendChild(s)}
 function boot(){load('automation_gates.js');load('qualification_ui.js')}
